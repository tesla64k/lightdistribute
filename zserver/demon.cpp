...
bool demon::ReadConf(std::string path)
{
	...
	if (nodetype == top)
	{
		...
		::GetPrivateProfileString(nodetype, TopCmdLine::proxy.c_str(), "", buf, sizeof buf, path.c_str());
		if (buf != "")
		...
		::GetPrivateProfileString(nodetype, TopCmdLine::stateDown.c_str(), "", buf, sizeof buf, path.c_str());
		if (buf != "")
	...
	else if (nodetype == ventilator)
	{
		...
		nodecmd.CreateCmd(nodemap, std::map<int, std::string>(), std::map<int, int>(),
